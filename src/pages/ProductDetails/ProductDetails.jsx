    ...
    <div className="container mx-auto p-4">
      <div className="flex flex-wrap -mx-4">
        ...
        <div className="w-full md:w-1/2 lg:w-2/3 px-4 mb-8">
          ...
          <p><span className="font-bold">Brand:</span> {product.brand_name}</p>
          <p><span className="font-bold">Product Type:</span> {product.type}</p>
          <p><span className="font-bold">Rating:</span> {product.rating}</p>
          <p><span className="font-bold">Availability:</span> {product.availability || "Not Available"}</p>
          <p><span className="font-bold">Color:</span> {product.color || "Not Available"}</p>
          <p><span className="font-bold">Warranty:</span> {product.warranty || "Not Available"}</p>
          <p><span className="font-bold">Shipping:</span> {product.shipping || "Not Available"}</p>
          <p className="text-gray-600 mb-4">{product.description}</p>
          <button onClick={handleAddToCart} className="bg-[#F5921D] text-white px-4 py-2 rounded-md mt-4">
            Add to Cart
          </button>
        </div>
      </div>
