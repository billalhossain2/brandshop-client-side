...
const MyCart = () => {
  ...
  useEffect(() => {
    ...
    fetch(`https://tech-store-server-one.vercel.app/cart/${displayName}`)
      .then((res) => res.json())
      .then((data) => {
        setCartItems(data);
        setIsLoading(false);
  ...
  const handleRemoveItem = (productId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        fetch(`https://tech-store-server-one.vercel.app/cart/${productId}`, {
          method: "DELETE",
        })
          .then((res) => res.json())
          .then((result) => {
        ...
        {cartItems.length === 0 ? (
          ...
          <table className="min-w-full">
            ...
            <tbody>
              {cartItems.map((item) => (
                <tr
                  className="border-b-2 border-solid border-[#f5b11d46]"
                  key={item.id}
                >
                  <td>
                    <img
