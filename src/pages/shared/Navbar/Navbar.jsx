    ...
    <>
      ...
      {user ? (
        <>
          ...
          <li>
            <div
              className="tooltip tooltip-open md:tooltip-bottom tooltip-right"
              data-tip={user?.displayName}
            >
              <NavLink className="relative" to="/profile">
              <img
                title={user?.displayName}
                className="w-12 h-12 rounded-full border-solid border-4 border- hover:border-[#F5921D]"
                src={user?.photoURL}
                alt="Profile Image"
              />
            </NavLink>
  ...
  return (
    <nav className="flex justify-between items-center md:px-20 mt-5 relative my-10 py-10">
      ...
      <div className="font-medium text-[18px] md:hidden">
        <FaBars
          onClick={handleBarsShow}
          className={`${
            show && "hidden"
          } text-4xl text-[#F5921D] border-2 border-solid border-[#F5921D]`}
        ></FaBars>
        <RxCross1
          onClick={handleBarsShow}
          className={`${
      ...
      {show && (
        <ul
          className={`top-${
            show ? "[50px]" : "[-500px]"
          } duration-300 md:hidden mt-10 absolute right-0 bg-zinc-900 w-full top-10 text-white py-2 pl-5 mobile-menu z-50`}
        >
          {navList}
        </ul>
      )}
